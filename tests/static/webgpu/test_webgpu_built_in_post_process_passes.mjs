...
async function testProbeCaptureExtensionIsRemoved() {
	const backend = new WebGPUBackend();
	backend.attach({
		surface: { canvas: {} },
		events: { emit: () => {} },
	});
	const extension = backend.extensions.getBackendExtension({
		id: "renderer.probe-capture",
	});
...
async function run() {
	...
	await testProbeCaptureExtensionIsRemoved();
