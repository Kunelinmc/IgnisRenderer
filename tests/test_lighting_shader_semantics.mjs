...
function testPBRNormalMapFallbackWithoutTangent() {
	...
	material.normalMap = new Texture(new Uint8ClampedArray([255, 128, 128, 255]), 1, 1)
...
function testLightProbeFallbackContributionFromDC() {
	...
	assert.ok(contribution, 'LightProbe should provide ambient fallback from SH DC term')
...
function testMaskShadowDepthWriteUsesAlphaCutoff() {
	...
	maskMaterial.map = new Texture(new Uint8ClampedArray([255, 255, 255, 255]), 1, 1)
	shadowMap.clear()
	rasterizer.drawDepthTriangle(makeTri(), shadowMap, maskMaterial)
