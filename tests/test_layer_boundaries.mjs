	...
	for (const fullPath of scanFiles) {
		...
		for (const specifier of specifiers) {
			...
			if (disallowedLegacyPath) {
				violations.push(
					`${relPath} imports legacy path "${specifier}"`
				)
			}
			...
			const inWebGPUBackendLayer = relPath.startsWith('src/core/backend/webgpu/')
			if (
				inWebGPUBackendLayer &&
				specifier.includes('software/lighting')
			) {
				violations.push(
