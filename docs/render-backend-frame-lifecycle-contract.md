...
- Each `FramePass` in `FrameContext.framePlan.backendPasses` must carry its
  applicable `dependsOn` stage ids.
...
- Backends must validate pass dependencies from `FramePass.dependsOn`; they
  must not read a mutable global dependency registry.
- `IRenderBackend` implementations must not expose `passExecutors`; executor
  routing must come from pipeline stage `kind`.
...
`IRenderBackend.passExecutors` is removed.
`FramePass.dependsOn` is required for renderer-generated frame plans and is used by backend pass-order validation.
`IRenderBackend.abortFrame` remains additive and optional.
`FrameContext.framePlan` remains optional for direct backend callers; when omitted, backends must skip global pass-order validation.
