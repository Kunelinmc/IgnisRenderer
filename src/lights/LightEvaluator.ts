...
import type { SceneLight } from './index'
...
export function createLightContribution(): MutableLightContribution {
	return {
		...
		intensity: 1,
		direction: { x: DEFAULT_DIRECTION.x, y: DEFAULT_DIRECTION.y, z: DEFAULT_DIRECTION.z },
	}
}
...
function evaluateAreaLight(
	...
	const cosLight = Math.max(0, Vector3.dot(Vector3.scale(normal, -1), direction))
	const distanceSq = distance * distance
	const attenuation =
...
function requireSurfacePosition(surface: SurfacePoint | null | undefined): IVector3 {
