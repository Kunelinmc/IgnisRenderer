if (!isClusteredLightingEnabled()) {
	...
	for (var i: u32 = 0u; i < areaCount; i = i + 1u) {
		for (
			...
			let radiance = areaLight.radiance;
			...
			let clearcoatTransmissionFresnel = select(
				0.0,
				fresnelSchlickScalar(ncDotV, 0.04),
				clearcoat > 0.0
			);
			let transmissionAttenuation =
				vec3<f32>(1.0 - clearcoatTransmissionFresnel * clearcoat);
			...
			if (nDotL > 0.0) {
				...
				if (maxSheenColor > 0.0) {
					...
					albedoSheenScaling =
						max(vec3<f32>(0.0), vec3<f32>(1.0) - sheenFresnel);
				}

				clearcoatAttenuation =
					vec3<f32>(1.0) - clearcoatFresnel * clearcoat;
