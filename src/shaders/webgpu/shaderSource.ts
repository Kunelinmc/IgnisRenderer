	...
	if (!cached) {
		cached = (async () => {
			if (isNodeRuntime()) {
				...
				return fsModule.readFile(new URL(nodeRelativePath, import.meta.url), 'utf8')
			}
...
export function loadSkyboxShaderSource(): Promise<string> {
	return loadShader('skybox', './skyboxShader.wgsl', () =>
		import('./skyboxShader.wgsl?raw')
	)
}

export function loadParticleShaderSource(): Promise<string> {
	return loadShader('particle', './particleShader.wgsl', () =>
