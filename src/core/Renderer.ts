...
export class Renderer extends EventEmitter<RendererEvents> {
	...
	public updateSH(): void {
		...
		const featureWorldMatrix = (this as Renderer & {
			params?: { worldMatrix?: Matrix4 }
		}).features?.worldMatrix
		const legacyWorldMatrix = (this as Renderer & {
			params?: { worldMatrix?: Matrix4 }
		}).params?.worldMatrix
		const worldMatrix = featureWorldMatrix || legacyWorldMatrix || Matrix4.identity()
