...
export class ParticleSimulationCore {
	...
	public trimToMaxParticles(runtime: SystemRuntimeState, maxParticles: number): void {
	...
	private _createParticle(
		...
		const emit = system.emit
		const lifetimeRange = overrides.lifetimeRange ?? emit.lifetimeRange ?? [0.5, 1.5]
		...
		return {
			...
			startSize: Math.max(0.001, this._randomRange(runtime, sizeRange[0], sizeRange[1])),
			startColor: {
				r: startColor.r,
				g: startColor.g,
				b: startColor.b,
	...
	private _resolveAABBCollider(
		...
		this._applyBounce(particle, hit.normal, collider.restitution, collider.damping)
	}
	...
	private _randomSpawnOffset(runtime: SystemRuntimeState, radius: number): IVector3 {
	...
	private _randomRange(runtime: SystemRuntimeState, min: number, max: number): number {
