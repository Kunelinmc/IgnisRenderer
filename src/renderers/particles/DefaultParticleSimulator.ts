...
import { PARTICLE_TRANSIENT_BATCHES_KEY, type FrameContext } from "../../pipeline/types"
...
export class DefaultParticleSimulator implements IParticleSimulator {
	...
	public emitRenderBatches(context: FrameContext): void {
		...
		for (const system of particleSystems) {
			...
			const batch = this._batchBuilder.buildBatch(system, runtime, context, sortRatio)
	...
	private _resolveLODLevel(
		...
		const projectedSize = this._computeProjectedSystemSizePx(system, context, distance)
	...
	private _resolveEffectiveMaxParticles(
		...
		const available = Math.min(system.maxParticles, this._maxParticlesPerSystem)
	...
	private _computeDistanceToCamera(system: ParticleSystem, context: FrameContext): number {
	...
	private _computeProjectedSystemSizePx(
		...
		const focalLength = (Math.max(1, context.attachments.height) * 0.5) / tanHalfFov
		return (estimatedRadius * focalLength * 2) / Math.max(0.001, distanceToCamera)
	}

	private _resolveDistanceTier(levels: ParticleLODLevel[], distance: number): number {
	...
	private _resolveScreenTier(levels: ParticleLODLevel[], projectedSize: number): number {
		for (let i = 0; i < levels.length; i++) {
