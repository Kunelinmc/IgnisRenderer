...
export interface WebGLParticleProgramLike {
...
export interface WebGLParticlePassHost {
	...
	getFogParams(): { params0: Float32Array; params1: Float32Array };
	...
	updateFogParams(options: FogOptions | undefined, enabled: boolean): void;
}
	...
	implements WebGLRenderPass<WebGLParticleRenderOptions>
{
	...
	public render(
		...
		this._warnSkippedMeshParticles(context);
		const framebuffer = options.framebuffer ?? this._host.getSceneFramebuffer();
		if (!framebuffer) {
			return;
		}
		...
		const previousOITPassMode = this._oitPassMode;
		this._oitPassMode = options.oitPassMode ?? 0;
		...
		this._host.updateFogParams(fogOptions, sceneFogEnabled);
		const fogParams = this._host.getFogParams();
		...
		try {
			for (const batch of batches) {
				...
				this._ensureCapacity(preflightCount);
				const instanceCount = this._writeInstances(batch);
				...
				gl.bufferSubData(
					gl.ARRAY_BUFFER,
					0,
					this._particleScratch.subarray(
						0,
						instanceCount * PARTICLE_INSTANCE_FLOATS
					)
				);

				const resolvedTexture = this._host.textures.getBaseColorTexture(
					batch.texture ?? null
				);
				...
				if (this._oitPassMode === 1) {
					gl.blendFuncSeparate(gl.ONE, gl.ONE, gl.ONE, gl.ONE);
				} else if (this._oitPassMode === 2) {
			...
			this._oitPassMode = previousOITPassMode;
			gl.blendFuncSeparate(
				gl.SRC_ALPHA,
				gl.ONE_MINUS_SRC_ALPHA,
				gl.ONE,
	...
	private _warnSkippedMeshParticles(context: FrameContext): void {
	...
	private _writeInstances(batch: ParticleRenderBatch): number {
	...
	private _ensureCapacity(requiredInstances: number): void {
		if (!this._particleInstanceBuffer || !this._particleVao) {
			return;
		}
		if (requiredInstances <= this._particleInstanceCapacity) {
			return;
		}
