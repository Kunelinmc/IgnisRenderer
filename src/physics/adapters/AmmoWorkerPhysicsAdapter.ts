...
const DEFAULT_WORKER_POOL_ID = "physics-ammo-worker";

function createUnsupportedSyncWorkerCallError(methodName: string): Error {
	return new Error(
		`AmmoWorkerPhysicsAdapter.${methodName} is not available in synchronous mode. Use stepAsync() via PhysicsSystem or switch to AmmoPhysicsAdapter.`
	);
}
...
export interface AmmoWorkerPhysicsAdapterOptions {
	enabled?: boolean;
...
export class AmmoWorkerPhysicsAdapter implements IPhysicsEngineAdapter {
	public readonly id = "ammo-worker";
	...
	private _pendingCommands: AmmoWorkerCommand[] = [];
	private _worldIds = new Set<string>();
	private _stepChain: Promise<void> = Promise.resolve();

	constructor(options: AmmoWorkerPhysicsAdapterOptions = {}) {
	...
	public initSync(): void {
		throw new Error(
			"AmmoWorkerPhysicsAdapter.initSync is not supported because worker initialization is asynchronous. Use init()."
		);
	}
	...
	public createBody(
		...
		if (this._usingFallbackAdapter) {
			this._fallbackAdapter.createBody(worldId, bodyId, descriptor, initialTransform);
			return;
		}
		this._assertInitialized();
		this._enqueueCommand({
	...
	public applyImpulse(worldId: string, bodyId: string, impulse: IVector3): void {
		if (this._usingFallbackAdapter) {
	...
	public destroyCharacterController(worldId: string, controllerId: string): void {
		if (this._usingFallbackAdapter) {
	...
	public isCharacterControllerGrounded(worldId: string, controllerId: string): boolean {
		if (this._usingFallbackAdapter) {
	...
	private _enqueueCommand(command: AmmoWorkerCommand): void {
		this._pendingCommands.push(command);
	}

	private _drainPendingCommands(): AmmoWorkerCommand[] {
	...
	private _assertInitialized(): void {
		...
		throw new Error(
			"AmmoWorkerPhysicsAdapter is not initialized. Call init() before using it."
		);
	}

	private async _dispatchToWorker<TResult>(
		payload: AmmoWorkerTaskPayload
	): Promise<TResult> {
		return this._scheduler.schedule<TResult, AmmoWorkerTaskPayload>(
			this._poolId,
			payload
		);
	}
