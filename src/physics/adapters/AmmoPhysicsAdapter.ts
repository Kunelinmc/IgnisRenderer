...
export class AmmoPhysicsAdapter implements IPhysicsEngineAdapter {
	...
	public createWorld(config: PhysicsWorldConfig): void {
		...
		try {
			...
			const dispatcher = this._newAmmo('btCollisionDispatcher', collisionConfig)
	...
	public createBody(
		...
		if (world.bodies.has(bodyId)) {
			throw new Error(`Physics body "${bodyId}" already exists in "${worldId}"`)
		}
		...
		try {
			...
			if (descriptor.linearVelocity) {
				this._setVector3(rigidBody, ['setLinearVelocity'], descriptor.linearVelocity)
			}
	...
	public setCharacterControllerStepHeight(
		...
		this._delegate.setCharacterControllerStepHeight(worldId, controllerId, value)
	}
	...
	private _applyBodyCcd(rigidBody: any, radius: number): void {
		...
		this._invoke(rigidBody, ['setCcdSweptSphereRadius'], [[Math.max(0.01, radius)]])
	}
	...
	private _setVector3(target: any, methods: string[], value: IVector3): void {
		...
		this._invoke(target, methods, [[vector], [value.x, value.y, value.z], [value]])
...
async function resolveAmmoModule(loaded: unknown): Promise<AmmoModuleLike | null> {
...
function readAmmoComponent(value: unknown, key: string, fallback: number): number {
