...
export class ShaderMaterial extends Material {
	...
	public resolveWebGPUProgram(mode: ShaderTargetMode): ResolvedWebGPUShaderProgram {
	...
	private _resolveStageCode(
		...
		const wgslSource = this._getProgramStageSource(this._webgpuWGSL, stage, mode)
		if (wgslSource) {
			return wgslSource
		}

		const glslSource = this._getProgramStageSource(this._webgpuGLSL, stage, mode)
		if (!glslSource) {
			throw new Error(
